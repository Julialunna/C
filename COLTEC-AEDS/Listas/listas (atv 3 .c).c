...
Lista* executa_soldado(Lista* p, Lista* paux) {
	paux->prox = p->prox;
	free(p);
	return paux->prox;
}

Lista* problema_Josephus(Lista* l, int numdepassos) {
	Lista* p = l;
	Lista* paux = NULL;
	int i = 0;

	do {
		for (i = 1; i < numdepassos - 1; i++) {
			p = p->prox;
		}
		paux = p;
		p = p->prox;
		p = executa_soldado(p, paux);
	} while (p != p->prox);

	printf("Posicao do soldado nao executado: %d", p->info);
	return p;
}